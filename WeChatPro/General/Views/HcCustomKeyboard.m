...
-(void)textViewShowView:(UIViewController *)viewController customKeyboardDelegate:(id)delegate
{
    ...
    self.mSecondaryBackView =[[UIView alloc]initWithFrame:CGRectMake(10, 10, WIDTH_SCREEN - 90, 30)];
    ...
    self.mTextView =[[UITextView alloc]initWithFrame:CGRectMake(1, 1, WIDTH_SCREEN - (320 - 228), 28)];
    ...
    UIImageView *talkImg =[[UIImageView alloc]initWithFrame:CGRectMake(WIDTH_SCREEN - 70, 16, 18, 18)];
    ...
    [self.mTalkBtn setTitle:@"确定" forState:UIControlStateNormal];
    [self.mTalkBtn addTarget:self action:@selector(forTalk) forControlEvents:UIControlEventTouchUpInside];
    [self.mTalkBtn setFrame:CGRectMake(WIDTH_SCREEN - 60, 16, 30, 18)];
...
{    
    ...
    [UIView animateWithDuration:0.3 animations:^{
        ...
        self.mHiddeView.alpha =0.0f;
        self.mSecondaryBackView.frame =CGRectMake(10, 10, WIDTH_SCREEN - 90, 30);
    } completion:^(BOOL finished) {
...
- (void)textDidChanged:(NSNotification *)notif //监听文字改变 换行时要更改输入框的位置
{
    ...
    self.mSecondaryBackView.frame =CGRectMake(10, 10, WIDTH_SCREEN - 90, selfHeight - 20);
    NSLog(@"文字改变");
}
